...
int main(int argc, char *argv[]) {
	...
	uint32_t X= sqrt(array_size_bytes/sizeof(double)); //width
	uint32_t Y=X; //height
	uint32_t size = X*Y;
	...
	for(runs = 1u; actual_runtime < minimal_runtime; runs = runs << 1u) {
		...
		for(uint64_t i = 0u; i < runs; i++) {
			...
			grid_target=temp;
			sprintf(filepath, "/home/hpc/rzku/hpcv720h/ex_01/grid%" PRIu64 ".ppm", i);
			if(i%10==0 && i<=500){
	...
	if(array_size_bytes==1024){
    	draw_grid(grid_source,X,Y,"/home/hpc/rzku/hpcv720h/ex_01/grid.ppm");
	}
	//TODO: calculate and print
	//length number of floating point additions in one vec_sum call
	//method is called for runs number of times
	runs = runs>>1u;
	mup_per_call = (X-2)*(Y-2);
	//number of 
